...
async function fetchWgerImages(): Promise<Map<string, string>> {
  ...
  while (url) {
    ...
    for (const ex of data.results) {
      ...
      if (en?.name) map.set(en.name.toLowerCase(), imgs[0].image)
    }
    url = data.next ?? null
  }
  return map
...
function findImage(name: string, wgerMap: Map<string, string>): string | undefined {
  const key = name.toLowerCase()
  if (wgerMap.has(key)) return wgerMap.get(key)
  // fuzzy: first wger name that contains our name or vice versa
  for (const [k, v] of wgerMap) {
    ...
    <div className="min-h-screen bg-[#0d1117] p-4">
      <div className="max-w-sm mx-auto">
        ...
        <div className="flex flex-col gap-2">
          {filtered.map(ex => {
            const imgUrl = findImage(ex.name, wgerMap)
